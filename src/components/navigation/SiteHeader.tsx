...
const NAV_CLASS = 'touch-target inline-flex items-center justify-center gap-2 rounded border px-3 text-sm font-medium transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-slate-50';

export const SiteHeader: React.FC<SiteHeaderProps> = ({ mode }) => (
  <header className="relative z-30 flex shrink-0 items-center justify-between gap-4 border-b border-slate-700 bg-slate-900 px-4 py-2 md:px-8">
    ...
    <nav aria-label="Ana gezinme" className="flex min-w-0 items-center gap-2 overflow-x-auto whitespace-nowrap">
      <AppLink to={getModePath('student')} current={mode === 'student'} className={`${NAV_CLASS} ${mode === 'student' ? 'border-chem-nonmetal bg-slate-800 text-slate-50' : 'border-slate-700 text-slate-300 hover:bg-slate-800'}`}>
        <GraduationCap className="h-4 w-4" aria-hidden="true" /> Öğrenci
      </AppLink>
      <AppLink to={getModePath('teacher')} current={mode === 'teacher'} className={`${NAV_CLASS} ${mode === 'teacher' ? 'border-chem-transition bg-slate-800 text-slate-50' : 'border-slate-700 text-slate-300 hover:bg-slate-800'}`}>
        <Presentation className="h-4 w-4" aria-hidden="true" /> Öğretmen
      </AppLink>
      <AppLink to={getLaboratoryPath('free')} className={`${NAV_CLASS} border-slate-700 bg-slate-950 text-slate-50 hover:bg-slate-800`}>
        Serbest Laboratuvar <ArrowUpRight className="h-4 w-4" aria-hidden="true" />
      </AppLink>
    </nav>
  </header>
);
