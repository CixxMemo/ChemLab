...
export const TeacherShareControls: React.FC<{ scenarioId: string }> = ({ scenarioId }) => {
  ...
  return <section aria-label="Öğrenciye deney paylaş" className="shrink-0 border-b border-slate-700 bg-slate-900 px-4 py-3 text-base text-slate-50">
    ...
    <p role="status" className="mt-1 text-slate-300">{status || 'Hesap gerekmez. Bağlantı yalnızca doğrulanmış deney ve isteğe bağlı görevi içerir.'}</p>
  </section>;
};
